...
export class MovieDetailsComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.moviesService.getById(this.id).subscribe(movie => {
      ...
      this.trailerURL = this.trandformYoutubeURLToEmbed(movie.trailer);
      if(movie.theaters) {
  ...
  trandformYoutubeURLToEmbed(url: string) : SafeResourceUrl | string {
    if(!url){
      return '';
    }
    ...
    let ampresandPosition = videoId.indexOf('&');
    if(ampresandPosition  !== -1) {
      videoId = videoId.substring(0 , ampresandPosition);
    }
  ...
  rate(rate: number) {
    this.ratingService.rate(this.id, rate).subscribe(() => {
      Swal.fire('Successful', 'Your rate has been recieved', 'success');
    })
  }
